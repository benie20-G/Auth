...
const { signupSchema, loginSchema, verificationSchema, acceptCodeSchema } = require("../middlewares/validator")
...
exports.signup = async (req, res) => {
    ...
    try {
        ...
        if (err) {
            res.status(401).json({ success: false, message: 'failed to register a user' })
        }
        ...
        console.log(error)
        res.status(404).json({ success: false, message: "failed to register a user" })
...
exports.login = async (req, res) => {
    ...
    try {
        ...
        if (err) {
            res.status(401).json({ success: false, message: 'failed to login a user' })
        }
        ...
        const isPasswordValid = await dohashcompare(password, existingUser.password)
...
exports.sendVerificationCode = async (req, res) => {
    ...
    try {
        ...
        if (info.accepted[0] === existingUser.email) {
            ...
            return res.status(200).json({ success: true, message: "code sent", data: result })
        }
...
exports.verifyVerificationCode = async (req, res) => {
   ...
   if(error){
    ...
    if(existingUser.verified){
    return res.status(400).json({success:false, message:" your account is arleady verified"})
    }
    console.log(existingUser.verificationCode)
    console.log(existingUser.verificationCodeValidation)
    if(!existingUser.verificationCode || !existingUser.verificationCodeValidation){
        return res.status(400).json({success:false, message:"somethin is wrong with the code"})
    }
